...
beforeAll(async () => {
  ...
  const manager = await User.create({ username: 'admin', password: 'pwd', email: 'a@test', roleId: managerRole._id })
  ...
  const s1 = await ReviewStage.create({ name: 'S1', order: 1, responsible: manager._id })
  await ReviewStage.create({ name: 'S2', order: 2, responsible: manager._id })

  await ReviewRecord.create({ assetId: productId, stageId: s1._id, completed: true })
})
...
describe('Product stages', () => {
  it('should return stages with completion status', async () => {
    const res = await request(app)
      .get(`/api/products/${productId}/stages`)
