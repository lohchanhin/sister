...
beforeAll(async () => {
  ...
  const res = await request(app).post("/api/auth/login").send({ username: "admin", password: "mypwd" })
  token = res.body.token

  const res2 = await request(app).post("/api/auth/login").send({ username: "user1", password: "pwd" })
  token2 = res2.body.token

  const res3 = await request(app).post("/api/auth/login").send({ username: "user2", password: "pwd" })
...
describe("Folder type", () => {
  ...
  it("default query should return 0", async () => {
    const res = await request(app).get("/api/folders").set("Authorization", `Bearer ${token}`).expect(200)
...
describe("Delete folder", () => {
  it("should remove subfolders and assets", async () => {
    ...
    await request(app).delete(`/api/folders/${folderId}`).set("Authorization", `Bearer ${token}`).expect(200)
