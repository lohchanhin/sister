...
<template>
  ...
  <Dialog v-model:visible="dialog" :header="editing ? '編輯平台' : '新增平台'" :modal="true" class="p-fluid w-full md:w-40rem">
    ...
    <div v-if="form.mode === 'custom'" class="field">
      ...
      <div class="flex flex-wrap align-items-center gap-2">
        ...
        <FormulaBuilder v-if="newFieldType === 'formula'" v-model="newFieldFormula" :fields="fieldSlugs"
          class="flex-1" />
      ...
      <OrderList v-model="form.fields" listStyle="height:auto" dataKey="slug" class="mt-2">
        ...
        <template #item="slotProps">
          <div class="flex justify-content-between align-items-center w-full gap-2">
            ...
            <Dropdown v-model="slotProps.item.type" :options="fieldTypeOptions" optionLabel="label" optionValue="value"
              class="w-12rem" />
            <FormulaBuilder v-if="slotProps.item.type === 'formula'" v-model="slotProps.item.formula"
              :fields="fieldSlugs" class="flex-1" />
  ...
  <Dialog v-model:visible="transferDialog" header="轉移平台" :modal="true" class="p-fluid w-full md:w-20rem">
    <div class="field">
      ...
      <Dropdown id="transferTarget" v-model="transferTarget" :options="clients" optionLabel="name" optionValue="_id"
        placeholder="選擇客戶" />
    </div>
    <template #footer>
...
const submit = async () => {
  // 生成合法且唯一的 slug（字母或 _ 开头，只含 [a-z0-9_]; 中文名会回退成 f_1, f_2...）
  const normalizeSlug = (s) => {
    if (!s) return ''
    let out = s.trim().toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_+|_+$/g, '')
    if (!out) out = ''                // 中文等 -> 空，后面回退
    if (/^[0-9]/.test(out)) out = 'f_' + out
    return out
  }

  // 在 submit() 开头清洗 & 去重（很关键）
  const used = new Set()
  form.value.fields.forEach((f, idx) => {
    const raw = f.slug || f.name || `f_${idx + 1}`
    let slug = normalizeSlug(raw)
    if (!slug) slug = `f_${idx + 1}`    // 中文名导致空 -> 回退
    let base = slug, k = 1
    while (used.has(slug)) slug = `${base}_${++k}`
    used.add(slug)
    f.slug = slug
  })
