    ...
    <div class="library-header">
      <div class="header-content">
        <div class="header-title">
          <h1 class="title-text">成品庫</h1>
          <p class="title-subtitle">管理您的最終成品與交付項目</p>
        </div>
...
import { fetchProducts, updateProduct, deleteProduct, updateProductsViewers, getProductUrl, startBatchDownload as startProductBatchDownload, getBatchDownloadProgress as getProductBatchDownloadProgress, reviewProduct, fetchProductStages, updateProductStage } from '../services/products'
  ...
  const poll = async (maxAttempts = 80, interval = 1500) => {
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (progress?.percent) progressStore.updateTaskProgress(taskId, progress.percent);
...
async function downloadSingleItem(item) {
  try {
    const url = await getProductUrl(item._id, true);
  ...
  confirm.require({
    ...
    accept: async () => {
      ...
      try {
        ...
        if (productIds.length) deletePromises.push(deleteProduct(productIds));
        if (folderIds.length) folderIds.forEach(id => deletePromises.push(deleteFolder(id)));
...
<style scoped>
/* Using the same styles as AssetLibrary.vue */
.library-container {
  padding: 0;
  max-width: none;
}
